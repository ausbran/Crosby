...
export function initMap() {
  ...
  const openModal = (listingId) => {
    ...
    if (listing.directions && listing.directions.trim() !== "") {
      directionsEl.querySelector('.directions-text').innerText = listing.directions;
      directionsEl.style.display = ""; // Show if not empty
    } else {
      directionsEl.style.display = "none"; // Hide if empty
    }
    // address
    ...
    if (listing.images && listing.images.length > 0) {
      listing.images.forEach((imageHtml, index) => {
        ...
        imageDiv.addEventListener("click", () =>
          openPhotoModal(listing, index)
        );
        modalImageContainer.appendChild(imageDiv);
      });
    }

    // Always call after DOM updates
    initSlider();
